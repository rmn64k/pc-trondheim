// ghost loading based on threadIdx.x. Requires NX_TILE = NY_TILE.
__global__ void
grad_kernel_x_load(const real * __restrict f, real * __restrict u, const real xfactor, const real yfactor,
    const real zfactor)
{
	...
	if (threadIdx.x < 2 * NGHOST) {
		int off =  -3 + ghostMul[threadIdx.x] * NX_TILE;
		gli.x = xli + off;
		gli.y = yli;
		gi.x  = xi + off;
		gi.y  = yi;
	}
	else if (threadIdx.x < 4 * NGHOST) {
		int adjidx = threadIdx.x - 2 * NGHOST;
		int off = -3 + ghostMul[adjidx] * NX_TILE - 2 * NGHOST;
		gli.x = yli;
		gli.y = xli + off;
		gi.x  = blockIdx.x * blockDim.x + yli;
		gi.y  = blockIdx.y * blockDim.y + xli + off;
	}
...
void
grad_x_load(vf3dgpu &f, vf3dgpu &u)
{
	grad_kernel_x_load<<<xy_tile.nblocks, xy_tile.nthreads>>>(f.mem(), u.mem(),
